...
This is the hardcoded algorithm with the game environment.
...
# Running Generation.
def run_generation(gen_num, spec_num):
    snake_block_size = 10
    game_over = False
    start_pos = 50
    ...
    # bias_s = random.randint(-50, 50) / 10
    # bias_l = random.randint(-50, 50) / 10
    # bias_r = random.randint(-50, 50) / 10
    bias_s = 3.8
    bias_l = -4.9
    bias_r = -2.9
    bias_s = 0
    ...
    while not game_over:
        ...
        new_food_dis = math.sqrt((x1[0] - foodx)**2 + (y1[0] - foody)**2)
        if new_food_dis <= old_food_dis:
            prox_score += 1
        else:
            prox_score += 0
    ...
    # print("Game Over")
    # print(bias_s, bias_l, bias_r)
    tot_score = prox_score + 5 * (fruit_score - 3)
    print("Score: " + str(fruit_score - 3))
    # print()
    # return fruit_score - 2
    return moves_cache, fruit_score - 3
...
#     print(i)
#     moves, score = run_generation(1, 1)
#     x_axis.append(i)
#     moves_cache.append(moves)
#     scores_cache.append([i, score])

# df = pd.DataFrame(moves_cache)
# writer = pd.ExcelWriter("E:/Side Projects/Snake AI/Data/moves_log.xlsx", engine='xlsxwriter')
# df.to_excel(writer, sheet_name="moves", index=False, header=False)
# writer.save()
#
# df = pd.DataFrame(scores_cache)
# writer = pd.ExcelWriter("E:/Side Projects/Snake AI/Data/scores_log.xlsx", engine='xlsxwriter')
# df.to_excel(writer, sheet_name="scores", index=False, header=False)
# writer.save()

scores_cache = []
for i in range(100):
    sim = run_generation(i, 1)
    scores_cache.append(sim[1])
